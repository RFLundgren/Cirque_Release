...
| Pulse insights | Navidrome with plugin support, build dated 2026-03-03 or later, plus the Pulse plugin. Enhanced source attribution is best with navidrome-experimental `stable` or `develop`. | Enables Pulse cards for weekly, monthly, and yearly listening recaps. |
...
- Pulse plugin and insight system: https://github.com/RFLundgren/pulse
...
Pulse is an optional insight system. It is separate from the normal Subsonic API and requires a Navidrome build with plugin support, currently documented by Pulse as a build dated 2026-03-03 or later, plus the Pulse plugin itself.

Pulse records scrobbles after it is installed, builds insight snapshots on a scheduler, and stores those snapshots in a `Pulse-Data` playlist. Cirque hides that technical playlist from the normal playlist view and reads the Pulse payload to show user-friendly insight cards.

Pulse can show weekly, monthly, and yearly listening recaps. Depending on the available plugin data, Cirque can surface total plays, listening minutes, top artists, top tracks, top albums, top genres, mood breakdowns, listening streaks, peak listening days and times, weekday versus weekend splits, hourly listening patterns, session counts, longest session, daily listening coverage, variety score, repeat ratio, artist/album/genre concentration, recent-addition listening, and track obsession.

Mood breakdowns require mood playlists on the server. These can be created manually or generated by the AI mood playlist/tagging workflow. Enhanced source/client attribution requires the enhanced scrobble attribution path in navidrome-experimental `stable` or `develop`; without that, Pulse can still generate listening recaps from ordinary scrobbles, but source-level cards may be absent or less detailed.

Pulse does not import old listening history. It starts building useful data only after the plugin is installed and scrobbles are recorded. If you have just installed Pulse, play music and wait for the scheduler to generate its first snapshot before expecting real Pulse cards in Cirque.

If you run standard Navidrome, do not install Pulse, or use an older Pulse plugin that does not emit newer fields, unsupported Pulse cards should stay hidden rather than showing empty or broken screens.
